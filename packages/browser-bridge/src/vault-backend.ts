...
export interface VaultBackend {
  ...
  // Capture is registration's mirror: the value is read off the page by the
  // engine, so the secret comes IN via `commitCapture` rather than being
  // generated by the backend. `commitCapture` consumes the handle.
  beginCapture?(req: unknown): Promise<unknown>;
  commitCapture?(id: string, secret: SecretHandle): Promise<unknown>;
...
export const CAPABILITY_TOOLS: Readonly<Record<keyof Capabilities, readonly string[]>> = {
  ...
  capture: ["begin_credential_capture"],
