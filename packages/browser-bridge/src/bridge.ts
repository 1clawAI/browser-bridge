...
/**
 * Binds a backend's capture methods to the core's capture engine.
 *
 * Capture is registration's mirror: the value is read off the page rather than
 * generated by the backend, so the secret comes IN through `commitCapture`. The
 * engine still knows nothing about which backend it is talking to.
 */
class CaptureAdapter {
  ...
  constructor(
    ...
    this.#backend = backend;
    const b = backend as unknown as {
      commitCapture?: (id: string, secret: SecretHandle) => Promise<{ entryId: string }>;
    ...
    this.#engine = new CaptureEngine({
      ...
      commit: (id, secret) => {
        if (!b.commitCapture) {
          throw new Error("backend advertises capture but cannot commit");
        }
        return b.commitCapture(id, secret);
      },
      cancel: async (id) => {
  ...
  async capture(siteId: string, targetId: string): Promise<ToolResult> {
    const b = this.#backend as unknown as {
      beginCapture?: (req: { siteId: string }) => Promise<Record<string, unknown>>;
    };
    if (!b.beginCapture) return { status: "error", message: "capture is not available" };

    const decision = await b.beginCapture({ siteId });
    if (decision.kind !== "capture_grant") {
      return { status: "denied", reason: String(decision.reason ?? "policy_denied") };
    }
    const outcome = await this.#engine.capture(targetId, decision as never);
    switch (outcome.status) {
      case "captured":
        return { status: "captured", entryId: outcome.entryId };
      case "denied":
        return { status: "denied", reason: outcome.reason };
      case "rejected":
        return { status: "rejected", reason: outcome.reason };
...
export async function startBridge(opts: BridgeOptions): Promise<BridgeHandle> {
  ...
  // The context a target belongs to. The proxy is authoritative (it placed the
  // agent's target); the event-derived map is a fallback for targets it did not
  // open. Shared by the fill and capture engines: both open a windowed page in
  // the agent's context so the site's logged-in session applies.
  const browserContextOf = (t: string) => proxyContextForTarget?.(t) ?? targetContexts.get(t);
  ...
  return {
    ...
    async callTool(name, args, observe) {
      return dispatchTool(name, args, {
        ...
        ...(captures ? { capture: (siteId: string, targetId: string) => captures.capture(siteId, targetId) } : {}),
